...
export default function BookAppointment({ onBooked, doctorId: propDoctorId }: BookAppointmentProps) {
  ...
  return (
    <form onSubmit={handleBook} className="space-y-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
      <h3 className="text-xl font-bold mb-2 text-gray-900">Book Appointment</h3>
            ...
            <SelectContent>
              {doctors.map((doc: any) => {
                const name =
                  ...
                  (doc.email ? doc.email.split("@")[0] : "Doctor")
                const spec = (doc.specialization || "N/A").toString()
                return (
